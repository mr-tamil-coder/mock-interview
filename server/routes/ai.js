...
router.post('/generate-question', authenticate, async (req, res) => {
  try {
    ...
    const question = await aiService.generateQuestion(difficulty, topic, previousQuestions);
...
// Process voice input
router.post('/voice-input', authenticate, upload.single('audio'), async (req, res) => {
  try {
    if (!req.file) {
